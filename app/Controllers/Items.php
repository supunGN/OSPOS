...
class Items extends Secure_Controller
{
     ...
     * @noinspection PhpUnused
     */
    public function getSubcategories(int $category_id): void
    {
        $subcategories = $this->item->get_custom_subcategories($category_id);
    ...
    public function getView(int $item_id = NEW_ENTRY): void    // TODO: Long function. Perhaps we need to refactor out some methods.
    {
        ...
        $data['custom_categories'] = ['' => lang('Items.none')];
        foreach ($this->item->get_custom_categories() as $row) {
            $data['custom_categories'][$row['id']] = $row['name'];
        }
        
        $data['custom_subcategories'] = ['' => lang('Items.none')];
        if (!empty($item_info->category_id)) {
            foreach ($this->item->get_custom_subcategories($item_info->category_id) as $row) {
                $data['custom_subcategories'][$row['id']] = $row['name'];
            }
        }
    ...
    public function postSave(int $item_id = NEW_ENTRY): void
    {
        ...
        $item_data = [
            ...
            'category_id'           => empty($this->request->getPost('category_id')) ? null : intval($this->request->getPost('category_id')),
            'subcategory_id'        => empty($this->request->getPost('subcategory_id')) ? null : intval($this->request->getPost('subcategory_id')),
            'item_type'             => $item_type,
